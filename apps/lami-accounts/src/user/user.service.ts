...
export class UserService {
  ...
  async adminCreateUser(newUser: User): Promise<User> {
    try {
      ...
      await this.cognitoIdentityProviderClient
        .send(adminCreateUserCommandInput)
        .then(() => {
          // Set user password...
  ...
  async setUserPassword(
    ...
    try {
      ...
      const { ChallengeName, Session } =
        await this.cognitoIdentityProviderClient.send(initiateAuthCommand);
