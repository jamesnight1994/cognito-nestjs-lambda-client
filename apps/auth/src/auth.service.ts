...
export class AuthService {
  ...
  async getAccessToken(client: AppClient, callback: Callback) {
    ...
    try {
      ...
      const tenant = await appDataSource.getRepository(App).findOneBy({
        client_id: client.client_id,
      });
      await appDataSource.destroy();
      console.log(tenant);
      const userPoolName = tenant.user_pool;
      const COGNITO_DOMAIN = `https://${userPoolName}.auth.eu-west-1.amazoncognito.com/oauth2/token`;
  ...
  async adminCreateUser(newUser: NewUser, callback: Callback) {
    const tenant = await this.getAppBy({
  ...
  async initiateAuth(user: User, callback: Callback) {
    ...
    const command: InitiateAuthCommand = new InitiateAuthCommand(input);
  ...
  private async getAppBy(app) {
    await appDataSource.initialize();
    const tenant = await appDataSource.getRepository(App).findOneBy(app);
    await appDataSource.destroy();
    return tenant;
  }
}
